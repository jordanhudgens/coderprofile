...
import uuid
import ptest
from pfunctions import question_finder 
...
class InitialResponseForm(models.ModelForm):
  class Meta:
    model = Response  
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.survey = survey
    super(InitialResponseForm, self).__init__(*args, **kwargs)
    self.uuid = uuid.uuid4().hex  

  def save(self, commit=True):
    # save the response object
    response = super(InitialResponseForm, self).save(commit=False)
